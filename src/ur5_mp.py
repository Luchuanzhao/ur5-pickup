...
class ur5_mp:
    def __init__(self):
        ...
        rospy.loginfo("Starting node moveit_cartesian_path") #cartesian:di ka er coordinate
    ...
    def execute(self):
        if self.track_flag:
            ...
            if len(self.pointx)>3:
                if len(self.pointx)==4:
                    x_speed = np.mean(np.asarray(self.pointx[1:4]) - np.asarray(self.pointx[0:3]))
                    wpose.position.x += 2 * x_speed
                    wpose.position.z = 0.05


                else:
                    if len(self.pointx)==5:
                        tracker.flag2 = 1
                        self.cxy_pub.publish(tracker)

                    if len(self.pointx)<6:
                        x_speed = np.mean(np.asarray(self.pointx[1:5]) - np.asarray(self.pointx[0:4]))
                        wpose.position.x += (x_speed-self.error_x*0.01/105)
            ...
            # Set the next waypoint to the right 0.5 meters
                ...
                wpose.position.x -= self.error_x*0.06/105
            ...
            if self.phase == 1:
                ...
                """
                #plan is the path computed
                #if fraction==1: it implys path was computed well
